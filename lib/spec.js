...
var knownHelpers, templates, rules, ruleNext; // eslint-disable-line no-unused-vars
...
rules = {
    ...
    'GS001-DEPR-CON-BC': {
        level: 'error',
        rule: 'Replace the <code>{{#if @blog.cover}}</code> helper with <code>{{#if @blog.cover_image}}</code>',
        fatal: true,
        details: 'The <code>cover</code> attribute was replaced with <code>cover_image</code>.<br>' +
    ...
    'GS002-ID-HELPER': {
        ...
        details:
                    'The output of <code>{{id}}</code> has changed between Ghost LTS and 1.0.0. ' +
    ...
    'GS010-PJ-AUT-EM-REQ': {
        ...
        details: 'Please add the property <code>"author.email"</code> to your <code>package.json</code>. E.g. <code>{"author": {"email": "hello@example.com"}}</code>.<br>' +
